...
namespace fs
{
    ...
    Path::Path(const std::string& path) : m_pointsToContent(false), m_details(false)
    {
        setPath(path);
    }
    ...
    void Path::setPath(const std::string& path)
    {
        ...
        if (pos == m_path.size() - 1) { m_pointsToContent = true; }
    }
    ...
    Path Path::resolve(const Path& path) const
    {
        ...
        if (path.isEmpty() && isEmpty()) { return Path(); }

        // Return input path if it is an absolute path or this path is empty
        if (path.isAbsolute() || isEmpty()) { return path; }
    ...
    std::string Path::resolved() const
    {
        ...
        while (std::getline(ss, name, '/'))
        {
            ...
            if (name.empty() && !parts.empty()) { continue; }
        ...
        for (size_t i = 0; i < numParts; i++)
        {
            ...
            else
            {
                ...
                if (!(i == 0 && absolute) && path != "..") { removableItems++; }
            }
        }
        ...
        for (size_t i = 0; i < stack.size(); i++)
        {
            ...
            // Add separator, except for the last item (unless it is also the beginning of an
            // absolute path)
            if (i + 1 < stack.size() || (i == 0 && absolute)) resolved += "/";
        }
    ...
    void Path::analyze() const
    {
        ...
        std::string name;
        while (std::getline(ss, name, '/')) { parts.push_back(name); }
        ...
        if (numParts > 1)
        {
            ...
            for (size_t i = 0; i < numParts - 1; i++) { m_directoryPath += parts[i]; }
