...
constexpr const char *Basename(const char *fname, int offset) {
    return offset == 0 || fname[offset - 1] == '/' || fname[offset - 1] == '\\' ? fname + offset
...
#define __FILENAME__ Basename(__FILE__, sizeof(__FILE__) - 1)
    ...
    #ifdef _MSC_VER
        #define IF_LOG(severity) __pragma(warning(push)) __pragma(warning(disable : 4127))
if (true) {
    ;
} else
...
#define LOG_FMT(severity, format, ...)                                                                                 \
    IF_LOG(severity)                                                                                                   \
    glue::log::get()->log(severity, __LINE__, __FILENAME__, format, ##__VA_ARGS__)
...
#define LOGD_FMT_IF(condition, format, ...) LOG_FMT_IF(glue::log::debug, condition, format, ##__VA_ARGS__)
#define LOGI_FMT_IF(condition, format, ...) LOG_FMT_IF(glue::log::info, condition, format, ##__VA_ARGS__)
#define LOGW_FMT_IF(condition, format, ...) LOG_FMT_IF(glue::log::warn, condition, format, ##__VA_ARGS__)
#define LOGE_FMT_IF(condition, format, ...) LOG_FMT_IF(glue::log::error, condition, format, ##__VA_ARGS__)
#define LOGF_FMT_IF(condition, format, ...) LOG_FMT_IF(glue::log::fatal, condition, format, ##__VA_ARGS__)
