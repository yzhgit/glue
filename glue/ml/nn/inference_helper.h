...
class TensorInfo {
  ...
  public:
    TensorInfo()
        : name(""), id(-1), tensor_type(kTensorTypeNone), is_nchw(true) {}
    ~TensorInfo() {}
  ...
  public:
    ...
    std::vector<int32_t>
        tensor_dims; // InputTensorInfo:   [In] The dimentions of tensor. (If
                     // empty at initialize, the size is updated from model
                     // info.) OutputTensorInfo: [Out] The dimentions of tensor
                     // is set from model information
...
class InputTensorInfo : public TensorInfo {
  public:
    enum {
        kDataTypeImage,
        kDataTypeBlobNhwc, // data_ which already finished preprocess(color
                           // conversion, resize, normalize_, etc.)
        kDataTypeBlobNchw,
    };

  public:
    InputTensorInfo()
        : data(nullptr), data_type(kDataTypeImage),
...
class OutputTensorInfo : public TensorInfo {
  public:
    ...
    float *GetDataAsFloat() { /* Returned pointer should be with const, but
                                 returning pointer without const is convenient
                                 to create cv::Mat */
...
#pragma omp parallel
                for (int32_t i = 0; i < GetElementNum(); i++) {
                    const uint8_t *val_uint8 =
                        static_cast<const uint8_t *>(data);
                    float val_float =
                        (val_uint8[i] - quant.zero_point) * quant.scale;
...
#pragma omp parallel
                for (int32_t i = 0; i < GetElementNum(); i++) {
                    ...
                    float val_float =
                        (val_int8[i] - quant.zero_point) * quant.scale;
                    data_fp32_[i] = val_float;
                }
            }
            return data_fp32_;
...
class InferenceHelper {
  ...
  public:
    ...
    virtual int32_t
    PreProcess(const std::vector<InputTensorInfo> &input_tensor_info_list) = 0;
    virtual int32_t
    Process(std::vector<OutputTensorInfo> &output_tensor_info_list) = 0;

  protected:
    void ConvertNormalizeParameters(InputTensorInfo &tensor_info);

    void PreProcessImage(int32_t num_thread,
                         const InputTensorInfo &input_tensor_info, float *dst);
    void PreProcessImage(int32_t num_thread,
                         const InputTensorInfo &input_tensor_info,
                         uint8_t *dst);
    void PreProcessImage(int32_t num_thread,
                         const InputTensorInfo &input_tensor_info, int8_t *dst);

    template <typename T>
    void PreProcessBlob(int32_t num_thread,
