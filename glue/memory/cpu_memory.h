...
{
public:
    MemoryAllocator() : m_data(nullptr), m_alignedData(nullptr) {}
    ...
    // Returns a pointer to an allocated memory. If memory size of allocated memory chuck is enough
    // for requested size then return a point from preallocated memory, otherwise allocate heap
    // memory
    ...
    // Deallocates a memory by input address. If a pointer points on allocated chuck of memory
    // inside the allocator then the allocator just removes a reference to such area without any
    // cost, otherwise heap allocation
    ...
    void free(_DataType* address)
    {
        ...
        {
            std::map<size_t, uint8_t>::iterator pos = m_allocatedChunk.find(
                static_cast<size_t>(reinterpret_cast<uint8_t*>(address) - m_alignedData));
...
private:
    ...
    // first parameter is an offset from preallocated memory, second parameter is a power of 2
    // (level)
    ...
    virtual void _allocate(size_t size)
    {
        ...
        if (m_size != size && size > 0)
        {
            if (!m_allocatedChunk.empty())
                throw std::logic_error("Cannot free a memory on CPU. Not all objects were "
                                       "previously deallocated from allocator.");
            ...
            m_alignedData = ((dataAddress % alignment) == 0)
                                ? m_data
                                : m_data + (alignment - (dataAddress % alignment));
