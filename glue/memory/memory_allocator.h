...
{
public:
    BaseMemoryAllocator() : m_size(0) {}

    virtual ~BaseMemoryAllocator() {}

    // Allocates a chunk of memory. We recommend to call this function only one time at the startup
    // of an application. Do not reallocate memory if some objects in your source code are allocated
    // through this allocator.
    void reserve(size_t size)
    {
        if (size == 0) throw std::logic_error("Memory size cannot be 0");
        ...
        while (size > 0)
        {
            ...
            if (usedSize == 0) m_freeChunk.resize(levelCount + 1u);
...
protected:
    ...
    void _merge(size_t offset, uint8_t from)
    {
        ...
        for (std::vector<std::set<size_t>>::iterator level = m_freeChunk.begin() + from;
             level < m_freeChunk.end(); ++level, memorySize <<= 1)
        {
