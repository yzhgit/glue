...
#include "glue/base/object_pool.hpp"
...
namespace test {
    ...
    namespace {
        ...
        void blockFillAndFree(PoolT& mp, size_t size)
        {
            ...
            for (size_t i = 0; i < size; ++i) { mp.delete_object(v[i]); }
        }
        ...
        void iterateFullBlocks(PoolT& mp, const size_t size, const size_t expected_blocks)
        {
            ...
            for (auto p : v) { mp.delete_object(p); }
    ...
    TEST_CASE("DynamicObjectPool delete all", "[dynamicpool")
    {
        ...
        CHECK(mp.calc_stats().num_blocks == 1u);
        CHECK(mp.calc_stats().num_allocations == 0u);
        for (size_t i = 0; i < 64; ++i) { mp.new_object(1 << i); }
        ...
        CHECK(mp.calc_stats().num_blocks == 1u);
        CHECK(mp.calc_stats().num_allocations == 0u);
        for (size_t i = 0; i < 64; ++i) { v[i] = mp.new_object(1 << i); }
        ...
        CHECK(mp.calc_stats().num_blocks == 1u);
        CHECK(mp.calc_stats().num_allocations == 0u);
        for (size_t i = 0; i < 128; ++i) { v[i] = mp.new_object(1 << i); }
