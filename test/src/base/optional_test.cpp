...
TEST_CASE("optional::optional()", "[ctor]") {
    ...
    SECTION("Has no value") { REQUIRE_FALSE(static_cast<bool>(opt)); }
}
...
TEST_CASE("optional::optional( const optional& )", "[ctor]") {
    ...
    SECTION("Copying a non-null optional") {
        ...
        SECTION("Has a value") { REQUIRE(static_cast<bool>(opt)); }

        SECTION("Value is the same as original") { REQUIRE(opt.value() == value); }
    }
}
...
TEST_CASE("optional::optional( value_type&& )", "[ctor]") {
    ...
    SECTION("Value is the same as original") { REQUIRE(opt.value() == expectedValue); }
}
...
TEST_CASE("optional::~optional()", "[dtor]") {
    ...
    SECTION("Destructor gets called") { REQUIRE(is_called); }
}
...
TEST_CASE("optional::operator=( nullopt_t )", "[assignment]") {
    SECTION("Assigning over non-null value") {
        ...
        SECTION("Calls destructor on previous value") { REQUIRE(is_called); }

        SECTION("Converts to null") { REQUIRE_FALSE(static_cast<bool>(opt)); }
    }
...
TEST_CASE("optional::operator bool()", "[observers]") {
    SECTION("Optional does not contain a value") {
        ...
        SECTION("Is implicitly false") { REQUIRE_FALSE(op); }
    }
    SECTION("Optional contains a value") {
        optional<int> op = 42;

        SECTION("Is implicitly true") { REQUIRE(op); }
    }
}
...
TEST_CASE("optional::swap( optional<T>& )", "[modifiers]") {
    SECTION("Both optionals are null") {
        ...
        SECTION("Values swapped") {
            SECTION("op1 is null") { REQUIRE(op1 == nullopt); }
            SECTION("op2 is null") { REQUIRE(op2 == nullopt); }
        }
    }
    ...
    SECTION("Both optionals are non-null") {
        ...
        SECTION("Values swapped") {
            SECTION("op1 contains op2's value") { REQUIRE(op1.value() == value2); }
            SECTION("op2 contains op1's value") { REQUIRE(op2.value() == value1); }
        }

        SECTION("Optionals are non-null") {
            SECTION("op1 is non-null") { REQUIRE(static_cast<bool>(op1)); }
            SECTION("op2 is non-null") { REQUIRE(static_cast<bool>(op2)); }
        }
    }
    ...
    SECTION("(*this) optional is null") {
        ...
        SECTION("Values swapped") {
            SECTION("op1 contains op2's value") { REQUIRE(!op1.has_value()); }
            SECTION("op2 contains op1's value") { REQUIRE(op2 == value1); }
        }
    }
    ...
    SECTION("other optional is null") {
        ...
        SECTION("Values swapped") {
            SECTION("op1 contains op2's value") { REQUIRE(op1 == value2); }
            SECTION("op2 contains op1's value") { REQUIRE(!op2.has_value()); }
        }
    }
}
