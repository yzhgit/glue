...
bool File::copyDirectoryTo(const File& newDirectory) const
{
    if (isDirectory() && newDirectory.createDirectory())
    {
        for (auto& f : findChildFiles(File::findFiles, false))
            if (!f.copyFileTo(newDirectory.getChildFile(f.getFileName()))) return false;

        for (auto& f : findChildFiles(File::findDirectories, false))
            if (!f.copyDirectoryTo(newDirectory.getChildFile(f.getFileName()))) return false;
...
File File::getChildFile(StringRef relativePath) const
{
    ...
    if (isAbsolutePath(r)) return File(String(r));
...
int File::findChildFiles(Array<File>& results, int whatToLookFor, bool searchRecursively,
                         const String& wildcard, FollowSymlinks followSymlinks) const
{
    int total = 0;

    for (const auto& di :
         RangedDirectoryIterator(*this, searchRecursively, wildcard, whatToLookFor, followSymlinks))
    {
...
//==============================================================================
File File::getNonexistentChildFile(const String& suggestedPrefix, const String& suffix,
