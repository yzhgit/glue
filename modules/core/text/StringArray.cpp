...
StringArray::StringArray(const StringArray& other) : strings(other.strings) {}

StringArray::StringArray(StringArray&& other) noexcept : strings(std::move(other.strings)) {}

StringArray::StringArray(Array<String>&& other) noexcept : strings(std::move(other)) {}

StringArray::StringArray(const String& firstValue) { strings.add(firstValue); }
...
bool StringArray::operator!=(const StringArray& other) const noexcept { return !operator==(other); }

void StringArray::swapWith(StringArray& other) noexcept { strings.swapWith(other.strings); }

void StringArray::clear() { strings.clear(); }

void StringArray::clearQuick() { strings.clearQuick(); }

const String& StringArray::operator[](int index) const noexcept
{
    if (isPositiveAndBelow(index, strings.size())) return strings.getReference(index);

    static String empty;
    return empty;
}

String& StringArray::getReference(int index) noexcept { return strings.getReference(index); }
...
{
    if (contains(newString, ignoreCase)) return false;
...
void StringArray::addArray(const StringArray& otherArray, int startIndex, int numElementsToAdd)
{
    ...
    while (--numElementsToAdd >= 0) strings.add(otherArray.strings.getReference(startIndex++));
}
...
void StringArray::set(int index, String newString) { strings.set(index, std::move(newString)); }
...
int StringArray::indexOf(StringRef stringToLookFor, bool ignoreCase, int i) const
{
    ...
    {
        for (; i < numElements; ++i)
            if (strings.getReference(i).equalsIgnoreCase(stringToLookFor)) return i;
    }
    else
    {
        for (; i < numElements; ++i)
            if (stringToLookFor == strings.getReference(i)) return i;
    }
...
//==============================================================================
void StringArray::remove(int index) { strings.remove(index); }
...
void StringArray::removeString(StringRef stringToRemove, bool ignoreCase)
{
    ...
    {
        for (int i = size(); --i >= 0;)
            if (strings.getReference(i).equalsIgnoreCase(stringToRemove)) strings.remove(i);
    }
    else
    {
        for (int i = size(); --i >= 0;)
            if (stringToRemove == strings.getReference(i)) strings.remove(i);
    }
}
...
void StringArray::removeEmptyStrings(bool removeWhitespaceStrings)
{
    ...
    {
        for (int i = size(); --i >= 0;)
            if (!strings.getReference(i).containsNonWhitespaceChars()) strings.remove(i);
    }
    else
    {
        for (int i = size(); --i >= 0;)
            if (strings.getReference(i).isEmpty()) strings.remove(i);
    }
}

void StringArray::trim()
{
    for (auto& s : strings) s = s.trim();
}
...
{
    auto last = (numberToJoin < 0) ? size() : jmin(size(), start + numberToJoin);

    if (start < 0) start = 0;

    if (start >= last) return {};

    if (start == last - 1) return strings.getReference(start);
    ...
    for (int i = start; i < last; ++i)
        bytesNeeded += strings.getReference(i).getCharPointer().sizeInBytes() -
    ...
    while (start < last)
    {
        ...
        if (!s.isEmpty()) dest.writeAll(s.getCharPointer());

        if (++start < last && separatorBytes > 0) dest.writeAll(separator.text);
    }
...
int StringArray::addTokens(StringRef text, StringRef breakCharacters, StringRef quoteCharacters)
{
    ...
    if (text.isNotEmpty())
    {
        ...
        {
            auto tokenEnd =
                CharacterFunctions::findEndOfToken(t, breakCharacters.text, quoteCharacters.text);
            strings.add(String(t, tokenEnd));
            ++num;

            if (tokenEnd.isEmpty()) break;
...
StringArray StringArray::fromTokens(StringRef stringToTokenise, StringRef breakCharacters,
...
void StringArray::appendNumbersToDuplicates(bool ignoreCase, bool appendNumberToFirstInstance,
                                            UTF8 preNumberString,
                                            UTF8 postNumberString)
{
    if (preNumberString.getAddress() == nullptr) preNumberString = UTF8(" (");

    if (postNumberString.getAddress() == nullptr) postNumberString = UTF8(")");
    ...
    for (int i = 0; i < size() - 1; ++i)
    {
        ...
        if (nextIndex >= 0)
        {
            ...
            if (appendNumberToFirstInstance)
                s = original + String(preNumberString) + String(++number) +
            ...
            while (nextIndex >= 0)
            {
                set(nextIndex, (*this)[nextIndex] + String(preNumberString) + String(++number) +
                                   String(postNumberString));
...
void StringArray::minimiseStorageOverheads() { strings.minimiseStorageOverheads(); }
