...
//==============================================================================
BigInteger::BigInteger() : allocatedSize(numPreallocatedInts)
{
    for (int i = 0; i < numPreallocatedInts; ++i) preallocated[i] = 0;
}

BigInteger::BigInteger(const int32 value)
    : allocatedSize(numPreallocatedInts), highestBit(31), negative(value < 0)
{
    preallocated[0] = (uint32) std::abs(value);

    for (int i = 1; i < numPreallocatedInts; ++i) preallocated[i] = 0;

    highestBit = getHighestBit();
}

BigInteger::BigInteger(const uint32 value) : allocatedSize(numPreallocatedInts), highestBit(31)
{
...
BigInteger::BigInteger(int64 value)
    : allocatedSize(numPreallocatedInts), highestBit(63), negative(value < 0)
{
    ...
    for (int i = 2; i < numPreallocatedInts; ++i) preallocated[i] = 0;
...
{
    for (int i = 0; i < numPreallocatedInts; ++i) std::swap(preallocated[i], other.preallocated[i]);
...
BigInteger& BigInteger::operator=(const BigInteger& other)
{
    ...
    {
        highestBit = other.getHighestBit();
        auto newAllocatedSize =
            (size_t) jmax((size_t) numPreallocatedInts, sizeNeededToHold(highestBit));
...
uint32* BigInteger::getValues() const noexcept
{
    ...
    return heapAllocation != nullptr ? heapAllocation : const_cast<uint32*>(preallocated);
}
...
{
    while (--numBits >= 0) setBit(startBit++, shouldBeSet);
}
...
//==============================================================================
bool BigInteger::isZero() const noexcept { return getHighestBit() < 0; }

bool BigInteger::isOne() const noexcept { return getHighestBit() == 0 && !negative; }

bool BigInteger::isNegative() const noexcept { return negative && !isZero(); }

void BigInteger::setNegative(const bool neg) noexcept { negative = neg; }

void BigInteger::negate() noexcept { negative = (!negative) && !isZero(); }
...
BigInteger& BigInteger::operator*=(const BigInteger& other)
{
    ...
    for (int i = 0; i <= t; ++i)
    {
        ...
        {
            auto uv =
                (uint64) totalValues[i + j] + (uint64) values[j] * (uint64) mValues[i] + (uint64) c;
...
BigInteger& BigInteger::operator|=(const BigInteger& other)
{
    ...
    if (other.highestBit >= 0)
    {
        ...
        while (--n >= 0) values[n] |= otherValues[n];
...
BigInteger& BigInteger::operator&=(const BigInteger& other)
{
    ...
    while (n > (int) other.allocatedSize) values[--n] = 0;
...
void BigInteger::shiftLeft(int bits, const int startBit)
{
    if (startBit > 0)
    {
        for (int i = highestBit; i >= startBit; --i) setBit(i + bits, (*this)[i]);

        while (--bits >= 0) clearBit(bits + startBit);
    }
    else
    {
        ...
        if (wordsToMove > 0)
        {
            ...
            for (size_t j = 0; j < wordsToMove; ++j) values[j] = 0;
...
void BigInteger::shiftRight(int bits, const int startBit)
{
    if (startBit > 0)
    {
        for (int i = startBit; i <= highestBit; ++i) setBit(i, (*this)[i + bits]);

        highestBit = getHighestBit();
    }
    else
    {
        if (bits > highestBit) { clear(); }
        else
        {
            auto wordsToMove = bitToIndex(bits);
            ...
            if (wordsToMove > 0)
            {
                for (size_t i = 0; i < top; ++i) values[i] = values[i + wordsToMove];

                for (size_t i = 0; i < wordsToMove; ++i) values[top + i] = 0;
...
void BigInteger::extendedEuclidean(const BigInteger& a, const BigInteger& b, BigInteger& x,
                                   BigInteger& y)
{
...
void BigInteger::inverseModulo(const BigInteger& modulus)
{
    ...
    while (b2.isNegative()) b2 += modulus;
...
String BigInteger::toString(const int base, const int minimumNumCharacters) const
{
    ...
    if (base == 2 || base == 8 || base == 16)
    {
        ...
        for (;;)
        {
            ...
            s = String::charToString((glue_wchar) (uint8) hexDigits[remainder]) + s;
        }
    }
    else if (base == 10)
    {
...
//==============================================================================
void writeLittleEndianBitsInBuffer(void* buffer, uint32 startBit, uint32 numBits,
