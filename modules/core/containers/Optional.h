...
GLUE_BEGIN_IGNORE_WARNINGS_MSVC(4702)
...
public:
    ...
    Optional(const Optional& other) : placeholder(), valid(other.valid)
    {
        if (valid) new (&storage) Value(*other);
    }
    ...
    Optional(const Optional<Other>& other) : placeholder(), valid(other.hasValue())
    {
        if (valid) new (&storage) Value(*other);
    }
    ...
    template <typename U = Value,
              typename = std::enable_if_t<
                  !std::is_same<std::decay_t<U>, Optional>::value &&
                  std::is_constructible<Value, U>::value && std::is_assignable<Value&, U>::value &&
    ...
    Optional&
    operator=(Optional<Other>&& other) noexcept(noexcept(std::declval<Optional>().assign(other)))
    {
        assign(other);
        return *this;
    }
    ...
    void reset()
    {
        if (std::exchange(valid, false)) operator*().~Value();
    }
    ...
    {
        return *this ? **this : std::forward<U>(fallback);
    }
...
private:
    ...
    template <typename Other>
    void assign(Optional<Other>& other) noexcept(
        noexcept(std::declval<Value&>() = std::move(*other)) && noexcept(
            std::declval<Optional>().constructFrom(other)))
    {
        if (valid)
        {
            if (other.hasValue())
