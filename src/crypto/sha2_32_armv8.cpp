...
void SHA_256::compress_digest_armv8(secure_vector<uint32_t> &digest,
                                    const uint8_t input8[], size_t blocks) {
    ...
    // Intermediate void* cast due to
    // https://llvm.org/bugs/show_bug.cgi?id=20670
    const uint32_t *input32 = reinterpret_cast<const uint32_t *>(
        reinterpret_cast<const void *>(input8));
