...
{
    // Don't instantiate ThreadPool when compile ThreadPool and only use 1
    // thread
    if (number <= 1) { return 1; }
    std::lock_guard<std::mutex> _l(gInitMutex);
    if (nullptr == gInstance) { gInstance = new ThreadPool(number); }
    ...
    for (int thread_index = 1; thread_index < thread_num_; ++thread_index)
    {
        workers_.emplace_back([this, thread_index]() {
            while (!stop_)
            {
                ...
                while (!(*tasks_.second[thread_index])) { std::this_thread::yield(); }
...
ThreadPool::~ThreadPool()
{
    ...
    for (auto c : tasks_.second) { delete c; }
}

void ThreadPool::AcquireThreadPool()
{
    if (nullptr == gInstance) { return; }
...
void ThreadPool::Enqueue(TASK_BASIC&& task)
{
    ...
    {
        for (int i = 0; i < task.second; ++i) { task.first(i, 0); }
        return;
    }
    ...
    for (int i = 1; i < work_size; ++i) { *(gInstance->tasks_.second[i]) = true; }
...
void ThreadPool::Enqueue(TASK_COMMON&& task)
{
    ...
    {
        for (int v = start; v < end; v += step) { std::get<0>(task)(v, 0); }
        return;
    }
    if (work_size > gInstance->thread_num_)
    {
