 ...
 */
void Image2TensorFpga::choose(const uint8_t* src, Tensor* dst, ImageFormat srcFormat,
                              ImageFormat dstFormat, LayoutType layout, int srcw, int srch,
                              int dstw, int dsth, float* means, float* scales)
{
    paddle::zynqmp::float16* output = dst->mutable_data<paddle::zynqmp::float16>();
    ...
    impl_(src, output, srcFormat, dstFormat, srcw, srch, dstw, dsth, means, scales);
}
...
{
    int channel = 3;
    if (srcFormat == ImageFormat::BGR || srcFormat == ImageFormat::RGB) { channel = 3; }
    ...
    int wc_align_size = paddle::zynqmp::align_to_x(srcw * channel, IMAGE_ALIGNMENT);

    int align_size = 1 * srch * wc_align_size;
    uint8_t* img =
        reinterpret_cast<uint8_t*>(paddle::zynqmp::fpga_malloc(align_size * sizeof(uint8_t)));
    ...
    if (srcFormat == ImageFormat::BGR) { preprocess_args.vedio_in_fomat = 3; }
    ...
    if (dstFormat == ImageFormat::BGR) { preprocess_args.vedio_out_fomat = 1; }
    ...
    int in_size = srch * paddle::zynqmp::align_to_x(srcw * channel, IMAGE_ALIGNMENT);
    paddle::zynqmp::fpga_flush(preprocess_args.input_image_address, in_size * sizeof(uint8_t));
    int ret = paddle::zynqmp::compute_preprocess(preprocess_args);
    if (ret == 0)
    {
        int output_size = dsth * paddle::zynqmp::align_to_x(dstw * 3, IMAGE_ALIGNMENT);
        paddle::zynqmp::fpga_invalidate(preprocess_args.output_image_address,
