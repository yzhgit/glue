...
void bgr_to_tensor_chw(const uint8_t* src, float* output, int width, int height, float* means,
                       float* scales);

void bgra_to_tensor_chw(const uint8_t* src, float* output, int width, int height, float* means,
                        float* scales);

void bgr_to_tensor_hwc(const uint8_t* src, float* output, int width, int height, float* means,
                       float* scales);

void bgra_to_tensor_hwc(const uint8_t* src, float* output, int width, int height, float* means,
                        float* scales);
 ...
 */
void Image2Tensor::choose(const uint8_t* src, Tensor* dst, ImageFormat srcFormat, DataLayout layout,
                          int srcw, int srch, float* means, float* scales)
{
    float* output = dst->mutable_data<float>();
    if (layout == DataLayout::NCHW && (srcFormat == BGR || srcFormat == RGB))
    {
        impl_ = bgr_to_tensor_chw;
    }
    else if (layout == DataLayout::NHWC && (srcFormat == BGR || srcFormat == RGB))
    {
        impl_ = bgr_to_tensor_hwc;
    }
    else if (layout == DataLayout::NCHW && (srcFormat == BGRA || srcFormat == RGBA))
    {
        impl_ = bgra_to_tensor_chw;
    }
    else if (layout == DataLayout::NHWC && (srcFormat == BGRA || srcFormat == RGBA))
    {
        impl_ = bgra_to_tensor_hwc;
    }
    else if ((layout == DataLayout::NHWC || layout == DataLayout::NCHW) && (srcFormat == GRAY))
    {
    ...
    LITE_PARALLEL_BEGIN(i, tid, height)
    {
        ...
        if (cnt > 0)
        {
            ...
            asm volatile("pld [%[inptr0]]                         @ preload a, 64byte\n"
                         "pld [%[inptr0], #64]                         @ preload a, "
                         "64byte\n"
                         "pld [%[inptr0], #128]                         @ preload a, "
                         "64byte\n"
                         "pld [%[inptr0], #192]                         @ preload a, "
                         "64byte\n"
                         "1: \n"
    ...
    LITE_PARALLEL_BEGIN(i, tid, height)
    {
        ...
        if (cnt > 0)
        {
            ...
            asm volatile("prfm   pldl1keep, [%[inptr0]]                \n"
                         ...
                         "st1 {v11.4s}, [%[outr2]], #16 \n"
                         "bne 1b \n"
                         : [inptr0] "+r"(din_ptr), [outr0] "+r"(ptr_b_h), [outr1] "+r"(ptr_g_h),
                           [outr2] "+r"(ptr_r_h), [cnt] "+r"(cnt)
