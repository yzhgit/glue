                              ...
                              std::vector<TensorInfo>& output_tensor_info_list)
{
    ...
    if (helper_type_ == kNcnnVulkan) { net_->opt.use_vulkan_compute = 1; }
    ...
    bin_filename =
        bin_filename.replace(bin_filename.find(".param"), std::string(".param").length(), ".bin\0");
...
int PredictorImpl::Finalize(void)
{
    ...
    in_mat_list_.clear();
    out_mat_list_.clear();
    if (helper_type_ == kNcnnVulkan) { ncnn::destroy_gpu_instance(); }
    return kRetErr;
}
...
int PredictorImpl::PreProcess(const std::vector<TensorInfo>& input_tensor_info_list)
{
    ...
    for (const auto& input_tensor_info : input_tensor_info_list)
    {
        ...
        if (input_tensor_info.data_type == TensorInfo::kDataTypeImage)
        {
            ...
            {
                pixel_type = (input_tensor_info.image_info.is_bgr) ? ncnn::Mat::PIXEL_BGR
                                                                   : ncnn::Mat::PIXEL_RGB;
                if (input_tensor_info.image_info.swap_color)
                {
                    pixel_type = (input_tensor_info.image_info.is_bgr) ? ncnn::Mat::PIXEL_BGR2RGB
                     ...
                     (input_tensor_info.GetChannel() == 1))
            {
                pixel_type = ncnn::Mat::PIXEL_GRAY;
                     ...
                     (input_tensor_info.GetChannel() == 1))
            {
                pixel_type = (input_tensor_info.image_info.is_bgr) ? ncnn::Mat::PIXEL_BGR2GRAY
            ...
            {
                /* Convert to blob */
                ncnn_mat = ncnn::Mat::from_pixels((uint8_t*) input_tensor_info.data, pixel_type,
        ...
        else if (input_tensor_info.data_type == TensorInfo::kDataTypeBlobNhwc)
        {
            ...
            ncnn_mat = ncnn::Mat::from_pixels(
                ...
                input_tensor_info.GetChannel() == 3 ? ncnn::Mat::PIXEL_RGB : ncnn::Mat::PIXEL_GRAY,
                input_tensor_info.GetWidth(), input_tensor_info.GetHeight());
        }
        else if (input_tensor_info.data_type == TensorInfo::kDataTypeBlobNchw)
        {
            ncnn_mat = ncnn::Mat(input_tensor_info.GetWidth(), input_tensor_info.GetHeight(),
