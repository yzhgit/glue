...
PredictorImpl::PredictorImpl() { num_threads_ = 1; }
    ...
    for (const auto& input_tensor_info : input_tensor_info_list) {
        ...
        if (input_tensor_info.data_type == TensorInfo::kDataTypeImage) {
            ...
            if (input_tensor_info.image_info.crop_width == input_tensor_info.GetWidth() &&
                ...
                /* Convert to blob */
                ncnn_mat = ncnn::Mat::from_pixels((uint8_t*)input_tensor_info.data,
                                                  pixel_type,
                                                  input_tensor_info.image_info.width,
                                                  input_tensor_info.image_info.height);
            } else {
                /* Convert to blob with resize */
                ncnn_mat = ncnn::Mat::from_pixels_resize((uint8_t*)input_tensor_info.data,
                                                         pixel_type,
                                                         input_tensor_info.image_info.width,
                                                         input_tensor_info.image_info.height,
            ...
            ncnn_mat = ncnn::Mat(input_tensor_info.GetWidth(),
                                 input_tensor_info.GetHeight(),
                                 input_tensor_info.GetChannel(),
                                 input_tensor_info.data);
        } else {
