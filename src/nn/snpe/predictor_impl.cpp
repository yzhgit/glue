                              ...
                              std::vector<TensorInfo>& output_tensor_info_list)
{
    ...
    if (use_user_supplied_buffers)
    {
        if (buffer_type == USERBUFFER_TF8 || buffer_type == USERBUFFER_TF16)
        {
            ...
            createInputBufferMap(*input_map_, application_input_buffers_, snpe_user_input_buffers_,
                                 snpe_, true, bit_width);
        }
                                                           ...
                                                           bool use_user_supplied_buffers)
{
    ...
    if (!logger_opt) { PRINT_E("SNPE failed to obtain logging interface.\n"); }
                                    ...
                                    std::vector<TensorInfo>& output_tensor_info_list)
{
    for (auto& input_tensor_info : input_tensor_info_list)
    {
        ...
        if (GetTensorInfo(snpe, input_tensor_info.name, dims) != 0) { return kRetErr; }
    ...
    {
        std::vector<int32_t> dims;
        if (GetTensorInfo(snpe, output_tensor_info.name, dims) != 0) { return kRetErr; }
