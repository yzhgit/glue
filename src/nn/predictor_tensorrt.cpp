...
int32_t PredictorTensorRt::Initialize(
    ...
    if (model_filename.find(".onnx") != std::string::npos) {
        is_onnx_model = true;
        trt_model_filename =
            trt_model_filename.replace(trt_model_filename.find(".onnx"),
                                       std::string(".onnx").length(), ".trt\0");
    ...
    if (is_trt_model) {
        ...
        if (stream) {
            ...
            copy(std::istream_iterator<char>(stream),
                 std::istream_iterator<char>(), back_inserter(buffer));
        }
        ...
        auto parser =
            std::unique_ptr<nvonnxparser::IParser>(nvonnxparser::createParser(
                *network, sample::gLogger.getTRTLogger()));
        ...
        nvinfer1::Dims4 image_dims{CAL_BATCH_SIZE, CAL_IMAGE_C, CAL_IMAGE_H,
                                   CAL_IMAGE_W};
        BatchStream calibration_stream(CAL_BATCH_SIZE, CAL_NB_BATCHES,
                                       image_dims, CAL_LIST_FILE, data_dirs);
        auto calibrator = std::unique_ptr<nvinfer1::IInt8Calibrator>(
            new Int8EntropyCalibrator2<BatchStream>(
                calibration_stream, 0, "_cal.txt",
                input_tensor_info_list[0].name.c_str()));
        ...
        std::ofstream ofs(std::string(trt_model_filename),
    ...
    if (AllocateBuffers(input_tensor_info_list, output_tensor_info_list) !=
        kRetOk) {
        return kRetErr;
    }
    ...
    for (const auto &input_tensor_info : input_tensor_info_list) {
        ...
        if (input_tensor_info.data_type == InputTensorInfo::kDataTypeImage) {
            ...
            if (input_tensor_info.tensor_type == TensorInfo::kTensorTypeFp32) {
                float *dst =
                    (float *)(buffer_list_cpu_[input_tensor_info.id].first);
                PreProcessImage(num_threads_, input_tensor_info, dst);
            } else if (input_tensor_info.tensor_type ==
                       TensorInfo::kTensorTypeUint8) {
                uint8_t *dst =
                    (uint8_t *)(buffer_list_cpu_[input_tensor_info.id].first);
                PreProcessImage(num_threads_, input_tensor_info, dst);
            } else if (input_tensor_info.tensor_type ==
                       TensorInfo::kTensorTypeInt8) {
                int8_t *dst =
                    (int8_t *)(buffer_list_cpu_[input_tensor_info.id].first);
                PreProcessImage(num_threads_, input_tensor_info, dst);
            } else {
            ...
            if (input_tensor_info.tensor_type == TensorInfo::kTensorTypeFp32) {
                ...
                PreProcessBlob<uint8_t>(num_threads_, input_tensor_info, dst);
                       ...
                       TensorInfo::kTensorTypeInt32) {
                int32_t *dst =
                    (int32_t *)(buffer_list_cpu_[input_tensor_info.id].first);
...
int32_t PredictorTensorRt::Process(
    ...
    for (int i = 0; i < (int)buffer_list_cpu_.size(); i++) {
        if (engine_->bindingIsInput(i)) {
            cudaMemcpyAsync(buffer_list_gpu_[i], buffer_list_cpu_[i].first,
                            buffer_list_cpu_[i].second, cudaMemcpyHostToDevice,
                            stream);
        }
    }
    ...
    for (int i = 0; i < (int)buffer_list_cpu_.size(); i++) {
        if (!engine_->bindingIsInput(i)) {
            cudaMemcpyAsync(buffer_list_cpu_[i].first, buffer_list_gpu_[i],
                            buffer_list_cpu_[i].second, cudaMemcpyDeviceToHost,
                            stream);
        }
    }
    ...
    (void)
        output_tensor_info_list; // no need to set output data, because the ptr
...
int32_t PredictorTensorRt::AllocateBuffers(
    ...
    for (int32_t i = 0; i < num_of_in_out; i++) {
        ...
        case nvinfer1::DataType::kINT32:
            ...
            buffer_list_cpu_.push_back(std::pair<void *, int32_t>(
                buffer_cpu, data_size * sizeof(float)));
        ...
        case nvinfer1::DataType::kINT8:
            ...
            buffer_list_cpu_.push_back(std::pair<void *, int32_t>(
                buffer_cpu, data_size * sizeof(int8_t)));
        ...
        if (engine_->bindingIsInput(i)) {
            for (auto &input_tensor_info : input_tensor_info_list) {
                int32_t id =
                    engine_->getBindingIndex(input_tensor_info.name.c_str());
                if (id == i) {
                    ...
                    if (input_tensor_info.tensor_dims.empty()) {
                        ...
                        if (static_cast<int32_t>(
                                input_tensor_info.tensor_dims.size()) !=
                            dims.nbDims) {
                    ...
                    if (((input_tensor_info.tensor_type ==
                          TensorInfo::kTensorTypeUint8) &&
                         (data_type == nvinfer1::DataType::kINT8)) ||
                        ((input_tensor_info.tensor_type ==
                          TensorInfo::kTensorTypeFp32) &&
            ...
            for (auto &output_tensor_info : output_tensor_info_list) {
                int32_t id =
                    engine_->getBindingIndex(output_tensor_info.name.c_str());
                if (id == i) {
                    ...
                    if (output_tensor_info.tensor_dims.empty()) {
                         ...
                         * size defined in the model */
                        if (static_cast<int32_t>(
                                output_tensor_info.tensor_dims.size()) !=
                            dims.nbDims) {
                            PRINT_E("Output Tensor dims doesn't match\n");
                            return kRetErr;
                        }
                        for (int32_t i = 0; i < dims.nbDims; i++) {
                            if (output_tensor_info.tensor_dims[i] !=
                                dims.d[i]) {
                    ...
                    if (((output_tensor_info.tensor_type ==
                          TensorInfo::kTensorTypeUint8) &&
                         (data_type == nvinfer1::DataType::kINT8)) ||
