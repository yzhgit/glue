    ...
    if (use_user_supplied_buffers) {
        if (buffer_type == USERBUFFER_TF8 || buffer_type == USERBUFFER_TF16) {
            ...
            createOutputBufferMap(*output_map_, application_output_buffers_,
                                  snpe_user_output_buffers_, snpe_, true,
                                  bit_width);
            createInputBufferMap(*input_map_, application_input_buffers_,
                                 snpe_user_input_buffers_, snpe_, true,
                                 bit_width);
        } else if (buffer_type == USERBUFFER_FLOAT) {
            createOutputBufferMap(*output_map_, application_output_buffers_,
                                  snpe_user_output_buffers_, snpe_, false,
                                  bit_width);
            if (user_buffer_source_type == CPUBUFFER) {
                createInputBufferMap(*input_map_, application_input_buffers_,
                                     snpe_user_input_buffers_, snpe_, false,
                                     bit_width);
            } else {
...
PredictorSnpe::Process(std::vector<OutputTensorInfo> &output_tensor_info_list) {
    ...
    for (auto &output_tensor_info : output_tensor_info_list) {
        output_tensor_info.data =
            application_output_buffers_.at(output_tensor_info.name).data();
    }
...
PredictorSnpe::CreateSnpe(const std::string &model_filename,
    ...
    std::unique_ptr<zdl::DlContainer::IDlContainer> container =
        zdl::DlContainer::IDlContainer::open(
            zdl::DlSystem::String(model_filename.c_str()));
    ...
    for (auto &input_tensor_info : input_tensor_info_list) {
        ...
        if (dims.size() == 0 && input_tensor_info.tensor_dims.size() == 0) {
            ...
            return kRetErr;
                   ...
                   input_tensor_info.tensor_dims.size() > 0) {
            // do nothing
        } else if (dims.size() > 0 &&
                   input_tensor_info.tensor_dims.size() == 0) {
    ...
    for (auto &output_tensor_info : output_tensor_info_list) {
        ...
        if (dims.size() == 0 && output_tensor_info.tensor_dims.size() == 0) {
            ...
            return kRetErr;
                   ...
                   output_tensor_info.tensor_dims.size() > 0) {
            // do nothing
        } else if (dims.size() > 0 &&
                   output_tensor_info.tensor_dims.size() == 0) {
            ...
            for (int32_t d = 0; d < dims.size(); d++) {
                if (dims[d] != output_tensor_info.tensor_dims[d]) {
                    PRINT_E("%s: Dim size doesn't match: %d vs %d\n",
                            output_tensor_info.name.c_str(), dims[d],
