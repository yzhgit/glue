...
#undef JNI_CLASS_MEMBERS
    ...
    {
        return glueString(LocalRef<jstring>((jstring) getEnv()->CallStaticObjectMethod(
            SystemClass, SystemClass.getProperty, javaString(name).get())));
    }
    ...
    static String getLocaleValue(bool isRegion)
    {
        ...
        return glueString(LocalRef<jstring>((jstring) stringResult));
    }

    static String getAndroidOsBuildValue(const char* fieldName)
    {
        return glueString(LocalRef<jstring>((jstring) getEnv()->GetStaticObjectField(
            AndroidBuild,
            getEnv()->GetStaticFieldID(AndroidBuild, fieldName, "Ljava/lang/String;"))));
    }
} // namespace AndroidStatsHelpers

//==============================================================================
SystemStats::OperatingSystemType SystemStats::getOperatingSystemType() { return Android; }
...
String SystemStats::getCpuVendor() { return AndroidStatsHelpers::getSystemProperty("os.arch"); }

String SystemStats::getCpuModel() { return readPosixConfigFileValue("/proc/cpuinfo", "Hardware"); }
...
int SystemStats::getPageSize() { return static_cast<int>(sysconf(_SC_PAGESIZE)); }

//==============================================================================
String SystemStats::getLogonName()
{
    if (const char* user = getenv("USER")) return UTF8(user);

    if (struct passwd* const pw = getpwuid(getuid())) return UTF8(pw->pw_name);
