...
namespace TimeHelpers
{
    ...
    static std::tm millisToUTC(int64 millis) noexcept
    {
        ...
        if (_gmtime64_s(&result, &millis) != 0) zerostruct(result);
        ...
        if (gmtime_r(&now, &result) == nullptr) zerostruct(result);
    ...
    {
#if GLUE_ANDROID
        using StringType = UTF8;
#elif GLUE_WINDOWS
        using StringType = UTF16;
#else
        using StringType = UTF32;
#endif
        ...
        for (size_t bufferSize = 256;; bufferSize += 256)
        {
            ...
            if (numChars > 0 || format.isEmpty())
                return String(StringType(buffer), StringType(buffer) + (int) numChars);
        }
    }
    ...
    static String getUTCOffsetString(int utcOffsetSeconds, bool includeSemiColon)
    {
        if (const auto seconds = utcOffsetSeconds)
        {
            ...
            return String::formatted(includeSemiColon ? "%+03d:%02d" : "%+03d%02d", minutes / 60,
                                     abs(minutes) % 60);
        }
...
Time GLUE_CALLTYPE Time::getCurrentTime() noexcept { return Time(currentTimeMillis()); }

//==============================================================================
uint32 glue_millisecondsSinceStartup() noexcept;

uint32 Time::getMillisecondCounter() noexcept
{
    auto now = glue_millisecondsSinceStartup();
...
void Time::waitForMillisecondCounter(uint32 targetTime) noexcept
{
    for (;;)
    {
        ...
        if (toWait > 2) { Thread::sleep(jmin(20, toWait >> 1)); }
        else
        {
...
//==============================================================================
String Time::toString(bool includeDate, bool includeTime, bool includeSeconds,
                      bool use24HourClock) const
{
    ...
    if (includeDate)
    {
        result << getDayOfMonth() << ' ' << getMonthName(true) << ' ' << getYear();

        if (includeTime) result << ' ';
    }
    ...
    if (includeTime)
    {
        ...
        result << (use24HourClock ? getHours() : getHoursInAmPmFormat()) << (mins < 10 ? ":0" : ":")
               << mins;
        ...
        if (!use24HourClock) result << (isAfternoon() ? "pm" : "am");
    }
...
String Time::getTimeZone() const
{
...
#if GLUE_WINDOWS && (GLUE_MSVC || GLUE_CLANG)
    _tzset();
    ...
    if (isDaylightSavingTime())
    {
        ...
        if (zone[0].length() > 3 && zone[0].containsIgnoreCase("daylight") &&
            zone[0].contains("GMT"))
            zone[0] = "BST";
    }
...
String Time::toISO8601(bool includeDividerCharacters) const
{
    return String::formatted(includeDividerCharacters ? "%04d-%02d-%02dT%02d:%02d:%06.03f"
                             ...
                             getYear(), getMonth() + 1, getDayOfMonth(), getHours(), getMinutes(),
                             getSeconds() + getMilliseconds() / 1000.0) +
           getUTCOffsetString(includeDividerCharacters);
}

static int parseFixedSizeIntAndSkip(String::CharPointerType& t, int numChars,
                                    char charToSkip) noexcept
{
    ...
    for (int i = numChars; --i >= 0;)
    {
        ...
        if (!isPositiveAndBelow(digit, 10)) return -1;

        ++t;
        n = n * 10 + digit;
    }

    if (charToSkip != 0 && *t == (glue_wchar) charToSkip) ++t;
...
Time Time::fromISO8601(StringRef iso)
{
    ...
    if (nextChar == '-' || nextChar == '+')
    {
        ...
        milliseconds +=
            nextChar == '-' ? offsetMs : -offsetMs; // NB: this seems backwards but is correct!
    }
...
String Time::getMonthName(int monthNumber, const bool threeLetterVersion)
{
    ...
    return String(threeLetterVersion ? shortMonthNames[monthNumber] : longMonthNames[monthNumber]);
}
...
String Time::getWeekdayName(int day, const bool threeLetterVersion)
{
    ...
    return String(threeLetterVersion ? shortDayNames[day] : longDayNames[day]);
}
...
{
    for (int i = 0; i < 12; ++i)
        if (m.equalsIgnoreCase(shortMonthNames[i])) return i;
...
Time Time::getCompilationDate()
{
    ...
    return Time(dateTokens[2].getIntValue(), getMonthNumberForCompileDate(dateTokens[0]),
                dateTokens[1].getIntValue(), timeTokens[0].getIntValue(),
