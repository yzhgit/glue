...
public:
    ...
    int32_t GetElementNum() const
    {
        ...
        for (const auto& dim : tensor_dims) { element_num *= dim; }
        return element_num;
    }
...
public:
    ...
    std::vector<int32_t> tensor_dims; // InputTensorInfo:   [In] The dimentions of tensor. (If
...
public:
    ...
    ~OutputTensorInfo()
    {
        if (data_fp32_ != nullptr) { delete[] data_fp32_; }
    }

    float* GetDataAsFloat()
    { /* Returned pointer should be with const, but
