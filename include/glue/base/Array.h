...
/**
    ...
    The Array class can be used to hold simple, non-polymorphic objects as well as primitive types -
   to do so, the class must fulfill these requirements:
    ...
    - it must be able to be relocated in memory by a memcpy without this causing any problems - so
      objects whose functionality relies on external pointers or references to themselves can not be
   used.

    You can of course have an array of pointers to any kind of object, e.g. Array<MyClass*>, but if
    you do this, the array doesn't take any ownership of the objects - see the OwnedArray class or
   the ReferenceCountedArray class for more powerful ways of holding lists of objects.
...
public:
    ...
    */
    ...
    {
        while (*values != TypeToCreateFrom()) add(*data++);
    }
    ...
    */
    ...
    Array(const ElementType& singleElementToAdd) { add(singleElementToAdd); }

    /** Initialises an Array of size 1 containing a single element. */
    Array(ElementType&& singleElementToAdd) { add(std::move(singleElementToAdd)); }
    ...
    */
    ...
    /** Returns one of the elements in the array.
        ...
        @param index    the index of the element being requested (0 is the first element in the
       array)
    ...
    */
    ...
    /** Returns a direct reference to one of the elements in the array, without checking the index
       passed in.
    ...
    */
    inline ElementType* getRawDataPointer() noexcept { return values.begin(); }

    /** Returns a pointer to the actual array data.
        This pointer will only be valid until the next time a non-const method
        is called on the array.
    */
    inline const ElementType* getRawDataPointer() const noexcept { return values.begin(); }

    //==============================================================================
    /** Returns a pointer to the first element in the array.
        This method is provided for compatibility with standard C++ iteration mechanisms.
    */
    inline ElementType* begin() noexcept { return values.begin(); }

    /** Returns a pointer to the first element in the array.
        This method is provided for compatibility with standard C++ iteration mechanisms.
    */
    inline const ElementType* begin() const noexcept { return values.begin(); }

    /** Returns a pointer to the element which follows the last element in the array.
        This method is provided for compatibility with standard C++ iteration mechanisms.
    */
    inline ElementType* end() noexcept { return values.end(); }

    /** Returns a pointer to the element which follows the last element in the array.
        This method is provided for compatibility with standard C++ iteration mechanisms.
    */
    inline const ElementType* end() const noexcept { return values.end(); }

    /** Returns a pointer to the first element in the array.
        This method is provided for compatibility with the standard C++ containers.
    */
    inline ElementType* data() noexcept { return begin(); }

    /** Returns a pointer to the first element in the array.
        This method is provided for compatibility with the standard C++ containers.
    */
    inline const ElementType* data() const noexcept { return begin(); }
    ...
    */
    int indexOf(ParameterType elementToLookFor) const
    {
        ...
        for (; e != endPtr; ++e)
            if (elementToLookFor == *e) return static_cast<int>(e - values.begin());
    ...
    */
    ...
    void addArray(const Type* elementsToAdd, int numElementsToAdd)
    {
        ...
        if (numElementsToAdd > 0) values.addArray(elementsToAdd, numElementsToAdd);
    }
    ...
    */
    ...
    void addNullTerminatedArray(const Type* const* elementsToAdd)
    {
        ...
        for (auto e = elementsToAdd; *e != nullptr; ++e) ++num;
    ...
    */
    ...
    /** Adds elements from another array to the end of this array.
        ...
        @param numElementsToAdd     how many elements to add from the other array. If this
                                    value is negative or greater than the number of available
       elements, all available elements will be copied.
        @see add
    */
    template <class OtherArrayType>
    ...
    */
    ...
    int addSorted(ElementComparator& comparator, ParameterType newElement)
    {
        const ScopedLockType lock(getLock());
        auto index =
            findInsertIndexInSortedArray(comparator, values.begin(), newElement, 0, values.size());
    ...
    */
    ...
    int indexOfSorted(ElementComparator& comparator, TargetValueType elementToLookFor) const
    {
        ...
        for (int s = 0, e = values.size();;)
        {
            ...
            if (comparator.compareElements(elementToLookFor, values[s]) == 0) return s;

            auto halfway = (s + e) / 2;

            if (halfway == s) return -1;
    ...
    */
    void remove(int indexToRemove)
    {
        ...
        if (isPositiveAndBelow(indexToRemove, values.size())) removeInternal(indexToRemove);
    }
    ...
    */
    void removeLast(int howManyToRemove = 1)
    {
        ...
        if (howManyToRemove > 0)
        {
            ...
            if (howManyToRemove > values.size()) howManyToRemove = values.size();
    ...
    */
    ...
    void removeValuesIn(const OtherArrayType& otherArray)
    {
        ...
        if (this == &otherArray) { clear(); }
        else
        {
            if (otherArray.size() > 0)
            {
                for (int i = values.size(); --i >= 0;)
                    if (otherArray.contains(values[i])) removeInternal(i);
            }
        }
    }
...
private:
    ...
    void minimiseStorageAfterRemoval()
    {
        if (values.capacity() > jmax(minimumAllocatedSize, values.size() * 2))
            values.shrinkToNoMoreThan(
                jmax(values.size(), jmax(minimumAllocatedSize, 64 / (int) sizeof(ElementType))));
    }
};
