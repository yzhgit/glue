...
class ByteOrder {
   public:
    ...
    /** Returns a garbled float which has the reverse byte-order of the
     * original. */
    static float swap(float value) noexcept;

    /** Returns a garbled double which has the reverse byte-order of the
     * original. */
    static double swap(double value) noexcept;

    //==============================================================================
    /** Swaps the byte order of a signed or unsigned integer if the CPU is
     * big-endian */
    ...
    //==============================================================================
    /** Constructs a 16-bit integer from its constituent bytes, in order of
     * significance. */
    constexpr static uint16_t makeInt(uint8_t leastSig, uint8_t mostSig) noexcept;

    /** Constructs a 32-bit integer from its constituent bytes, in order of
     * significance. */
    constexpr static uint32_t makeInt(uint8_t leastSig,
                                      uint8_t byte1,
                                      uint8_t byte2,
                                      uint8_t mostSig) noexcept;

    /** Constructs a 64-bit integer from its constituent bytes, in order of
     * significance. */
    constexpr static uint64_t makeInt(uint8_t leastSig,
                                      uint8_t byte1,
                                      uint8_t byte2,
                                      uint8_t byte3,
                                      uint8_t byte4,
...
constexpr inline uint32_t ByteOrder::makeInt(uint8_t b0,
                                             uint8_t b1,
                                             uint8_t b2,
                                             uint8_t b3) noexcept {
...
constexpr inline uint64_t ByteOrder::makeInt(uint8_t b0,
                                             uint8_t b1,
                                             uint8_t b2,
                                             uint8_t b3,
                                             uint8_t b4,
...
constexpr inline uint32_t ByteOrder::littleEndianInt(const void* bytes) noexcept {
    return makeInt(static_cast<const uint8_t*>(bytes)[0],
                   static_cast<const uint8_t*>(bytes)[1],
                   static_cast<const uint8_t*>(bytes)[2],
                   static_cast<const uint8_t*>(bytes)[3]);
}
constexpr inline uint64_t ByteOrder::littleEndianInt64(const void* bytes) noexcept {
    return makeInt(static_cast<const uint8_t*>(bytes)[0],
                   ...
                   static_cast<const uint8_t*>(bytes)[2],
                   static_cast<const uint8_t*>(bytes)[3],
                   static_cast<const uint8_t*>(bytes)[4],
                   static_cast<const uint8_t*>(bytes)[5],
                   static_cast<const uint8_t*>(bytes)[6],
                   static_cast<const uint8_t*>(bytes)[7]);
}
...
constexpr inline uint32_t ByteOrder::bigEndianInt(const void* bytes) noexcept {
    return makeInt(static_cast<const uint8_t*>(bytes)[3],
                   static_cast<const uint8_t*>(bytes)[2],
                   static_cast<const uint8_t*>(bytes)[1],
                   static_cast<const uint8_t*>(bytes)[0]);
}
constexpr inline uint64_t ByteOrder::bigEndianInt64(const void* bytes) noexcept {
    return makeInt(static_cast<const uint8_t*>(bytes)[7],
                   static_cast<const uint8_t*>(bytes)[6],
                   static_cast<const uint8_t*>(bytes)[5],
                   static_cast<const uint8_t*>(bytes)[4],
                   static_cast<const uint8_t*>(bytes)[3],
