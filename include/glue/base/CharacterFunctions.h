...
//==============================================================================
#if GLUE_WINDOWS && !defined(DOXYGEN)
    #define GLUE_NATIVE_WCHAR_IS_UTF8 0
    #define GLUE_NATIVE_WCHAR_IS_UTF16 1
    #define GLUE_NATIVE_WCHAR_IS_UTF32 0
#else
    /** This macro will be set to 1 if the compiler's native wchar_t is an 8-bit type. */
    #define GLUE_NATIVE_WCHAR_IS_UTF8 0
    /** This macro will be set to 1 if the compiler's native wchar_t is a 16-bit type. */
    #define GLUE_NATIVE_WCHAR_IS_UTF16 0
    /** This macro will be set to 1 if the compiler's native wchar_t is a 32-bit type. */
    #define GLUE_NATIVE_WCHAR_IS_UTF32 1
#endif

#if GLUE_NATIVE_WCHAR_IS_UTF32 || DOXYGEN
/** A platform-independent 32-bit unicode character type. */
using glue_wchar = wchar_t;
#else
using glue_wchar = uint32;
#endif

#ifndef DOXYGEN
    /** This macro is deprecated, but preserved for compatibility with old code. */
    #define GLUE_T(stringLiteral) (L##stringLiteral)
#endif

#if GLUE_DEFINE_T_MACRO
    /** The 'T' macro is an alternative for using the "L" prefix in front of a string literal.
        ...
        GLUE_DEFINE_T_MACRO = 1. The fastest, most portable and best way to write your string
    ...
    */
    #define T(stringLiteral) GLUE_T(stringLiteral)
#endif
...
/**
    ...
    @see String, UTF8, UTF16, UTF32

    @tags{Core}
*/
class GLUE_API CharacterFunctions
{
public:
    //==============================================================================
    /** Converts a character to upper-case. */
    static glue_wchar toUpperCase(glue_wchar character) noexcept;
    /** Converts a character to lower-case. */
    static glue_wchar toLowerCase(glue_wchar character) noexcept;

    /** Checks whether a unicode character is upper-case. */
    static bool isUpperCase(glue_wchar character) noexcept;
    /** Checks whether a unicode character is lower-case. */
    static bool isLowerCase(glue_wchar character) noexcept;

    /** Checks whether a character is whitespace. */
    static bool isWhitespace(char character) noexcept;
    /** Checks whether a character is whitespace. */
    static bool isWhitespace(glue_wchar character) noexcept;

    /** Checks whether a character is a digit. */
    static bool isDigit(char character) noexcept;
    /** Checks whether a character is a digit. */
    static bool isDigit(glue_wchar character) noexcept;

    /** Checks whether a character is alphabetic. */
    static bool isLetter(char character) noexcept;
    /** Checks whether a character is alphabetic. */
    static bool isLetter(glue_wchar character) noexcept;

    /** Checks whether a character is alphabetic or numeric. */
    static bool isLetterOrDigit(char character) noexcept;
    /** Checks whether a character is alphabetic or numeric. */
    static bool isLetterOrDigit(glue_wchar character) noexcept;
    ...
    */
    ...
    /** Checks whether a character is a printable character, i.e. alphabetic, numeric,
        a punctuation character or a space.
    */
    static bool isPrintable(glue_wchar character) noexcept;

    /** Returns 0 to 16 for '0' to 'F", or -1 for characters that aren't a legal hex digit. */
    static int getHexDigitValue(glue_wchar digit) noexcept;

    /** Converts a byte of Windows 1252 codepage to unicode. */
    static glue_wchar getUnicodeCharFromWindows1252Codepage(uint8 windows1252Char) noexcept;
    ...
    */
    ...
    static double readDoubleValue(CharPointerType& text) noexcept
    {
        ...
        constexpr const int bufferSize =
            maxSignificantDigits + 7 + 1; // -.E-XXX and a trailing null-terminator
...
#if GLUE_MINGW
        // MinGW does not have access to the locale functions required for strtold, so we parse the
        // doubles ourselves. There are some edge cases where the least significant digit will be
        // wrong!
        ...
        for (;;)
        {
            if (text.isDigit())
            {
                ...
                if (decPointIndex != 0) exponentAdjustment[1]++;

                if (numSignificantDigits == 0 && digit == 0) continue;
                ...
                if (++numSignificantDigits > maxSignificantDigits)
                {
                    ...
                    {
                        ++text;
                        if (decPointIndex == 0) exponentAdjustment[0]++;
                    }
                }
                else
                {
                    const auto maxAccumulatorValue =
                        (double) ((std::numeric_limits<unsigned int>::max() - 9) / 10);
                    if (accumulator[decPointIndex] > maxAccumulatorValue)
                    {
                        result[decPointIndex] =
                            mulexp10(result[decPointIndex], exponentAccumulator[decPointIndex]) +
                            accumulator[decPointIndex];
        ...
        if ((c == 'e' || c == 'E') && digitsFound)
        {
            ...
            while (text.isDigit()) exponent = (exponent * 10) + ((int) text.getAndAdvance() - '0');

            if (negativeExponent) exponent = -exponent;
        }

        auto r = mulexp10(result[0], exponent + exponentAdjustment[0]);
        if (decPointIndex != 0) r += mulexp10(result[1], exponent - exponentAdjustment[1]);
        ...
        for (;;)
        {
            if (text.isDigit())
            {
                ...
                {
                    if (numSigFigs >= maxSignificantDigits) continue;
                }
                else
                {
        ...
        if (c == 'e' || c == 'E')
        {
            ...
            *writePtr++ = 'e';
            ...
            while (text.isDigit())
            {
                ...
                if (digit != 0 || exponent != 0) exponent = (exponent * 10) + digit;
            }

            if (text == startOfExponentDigits) text = startOfExponent;
    ...
    struct HexParser
    {
        ...
        static ResultType parse(CharPointerType t) noexcept
        {
            ...
            while (!t.isEmpty())
            {
                ...
                if (hexValue >= 0)
                    result =
                        static_cast<ResultType>(result << 4) | static_cast<ResultType>(hexValue);
            }
    ...
    static size_t lengthUpTo(CharPointerType text, const size_t maxCharsToCount) noexcept
    {
        ...
        while (len < maxCharsToCount && text.getAndAdvance() != 0) ++len;
    ...
    static size_t lengthUpTo(CharPointerType start, const CharPointerType end) noexcept
    {
        ...
        while (start < end && start.getAndAdvance() != 0) ++len;
    ...
    {
        while (auto c = src.getAndAdvance()) dest.write(c);
    ...
    template <typename DestCharPointerType, typename SrcCharPointerType>
    static size_t copyWithDestByteLimit(DestCharPointerType& dest, SrcCharPointerType src,
                                        size_t maxBytesToWrite) noexcept
    {
        auto startAddress = dest.getAddress();
        auto maxBytes = (ssize_t) maxBytesToWrite;
        maxBytes -= (ssize_t) sizeof(
            typename DestCharPointerType::CharType); // (allow for a terminating null)
    ...
    template <typename DestCharPointerType, typename SrcCharPointerType>
    static void copyWithCharLimit(DestCharPointerType& dest, SrcCharPointerType src,
    ...
    static int compare(glue_wchar char1, glue_wchar char2) noexcept
    {
        if (auto diff = static_cast<int>(char1) - static_cast<int>(char2)) return diff < 0 ? -1 : 1;
    ...
    static int compare(CharPointerType1 s1, CharPointerType2 s2) noexcept
    {
        for (;;)
        {
            ...
            if (auto diff = compare(c1, s2.getAndAdvance())) return diff;
    ...
    static int compareIgnoreCase(glue_wchar char1, glue_wchar char2) noexcept
    {
    ...
    static int compareIgnoreCase(CharPointerType1 s1, CharPointerType2 s2) noexcept
    {
        for (;;)
        {
            ...
            if (auto diff = compareIgnoreCase(c1, s2.getAndAdvance())) return diff;
    ...
    template <typename CharPointerType1, typename CharPointerType2>
    static int compareIgnoreCaseUpTo(CharPointerType1 s1, CharPointerType2 s2,
                                     int maxChars) noexcept
    {
        while (--maxChars >= 0)
        {
    ...
    */
    template <typename CharPointerType1, typename CharPointerType2>
    static int indexOf(CharPointerType1 textToSearch,
                       const CharPointerType2 substringToLookFor) noexcept
    {
        ...
        for (;;)
        {
            if (textToSearch.compareUpTo(substringToLookFor, substringLength) == 0) return index;

            if (textToSearch.getAndAdvance() == 0) return -1;
    ...
    */
    template <typename CharPointerType1, typename CharPointerType2>
    static CharPointerType1 find(CharPointerType1 textToSearch,
                                 const CharPointerType2 substringToLookFor) noexcept
    {
        auto substringLength = (int) substringToLookFor.length();
    ...
    */
    template <typename CharPointerType>
    static CharPointerType find(CharPointerType textToSearch,
                                const glue_wchar charToLookFor) noexcept
    {
        for (;; ++textToSearch)
        {
            auto c = *textToSearch;

            if (c == charToLookFor || c == 0) break;
        }
    ...
    */
    ...
    static int indexOfIgnoreCase(CharPointerType1 haystack, const CharPointerType2 needle) noexcept
    {
        ...
        for (;;)
        {
            if (haystack.compareIgnoreCaseUpTo(needle, needleLength) == 0) return index;

            if (haystack.getAndAdvance() == 0) return -1;
    ...
    */
    template <typename Type>
    static int indexOfChar(Type text, const glue_wchar charToFind) noexcept
    {
        int i = 0;

        while (!text.isEmpty())
        {
            if (text.getAndAdvance() == charToFind) return i;
    ...
    */
    template <typename Type>
    static int indexOfCharIgnoreCase(Type text, glue_wchar charToFind) noexcept
    {
        ...
        while (!text.isEmpty())
        {
            if (text.toLowerCase() == charToFind) return i;
