...
GLUE_API void SetLogLevel(LogLevel level);
...
GLUE_API LogLevel GetLogLevel(std::string module);
...
GLUE_API std::string GetLogLevelName(LogLevel level, bool pad = false);
...
GLUE_API void LogToFile(const std::string& path, bool append = false);
...
GLUE_API void SetLoggerChannel(std::shared_ptr<BaseLoggerChannel> loggerChannel);

/// \brief Get the current logger channel.
GLUE_API std::shared_ptr<BaseLoggerChannel> GetLoggerChannel();
