...
#include "glue/base/in_place.hpp" // in_place
...
template <typename E>
constexpr bool operator==(const unexpected_type<E>& lhs, const unexpected_type<E>& rhs);
template <typename E>
constexpr bool operator!=(const unexpected_type<E>& lhs, const unexpected_type<E>& rhs);
template <typename E>
constexpr bool operator<(const unexpected_type<E>& lhs, const unexpected_type<E>& rhs);
                                       ...
                                       T, E>
{
    ...
    template <typename U, typename G>
    static constexpr bool is_moveable =
        std::is_move_constructible<U>::value&& std::is_move_assignable<U>::value&&
            std::is_move_constructible<G>::value&& std::is_move_assignable<G>::value;

    template <typename U, typename V>
    static constexpr bool is_value_assignable =
        std::is_constructible<U, V>::value&& std::is_assignable<U, V>::value;
...
public:
    ...
    template <typename UError = E,
              typename = std::enable_if_t<std::is_copy_constructible<UError>::value>>
...
public:
    ...
    ///
    /// \param other the other expected
    expected(enable_overload_if<std::is_copy_constructible<E>::value, const expected&> other);
    expected(disable_overload_if<std::is_copy_constructible<E>::value, const expected&> other) =
        delete;
    ...
    ///
    /// \param other the other expected
    expected(enable_overload_if<std::is_move_constructible<E>::value, expected&&> other);
    expected(disable_overload_if<std::is_move_constructible<E>::value, expected&&> other) = delete;
...
public:
    ...
    /// \param fn the function to invoke with this
    /// \return The expected result of the function invoked
    template <typename Fn, typename = std::enable_if_t<is_invocable<Fn>::value>>
...
template <typename T, typename E>
constexpr bool operator==(const expected<T, E>& x, const unexpected_type<E>& e);
template <typename T, typename E>
constexpr bool operator==(const unexpected_type<E>& e, const expected<T, E>& x);
template <typename T, typename E>
constexpr bool operator!=(const expected<T, E>& x, const unexpected_type<E>& e);
template <typename T, typename E>
constexpr bool operator!=(const unexpected_type<E>& e, const expected<T, E>& x);
