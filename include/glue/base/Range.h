...
public:
    ...
    GLUE_NODISCARD constexpr static Range emptyRange(const ValueType start) noexcept
    {
    ...
    */
    GLUE_NODISCARD constexpr Range withStart(const ValueType newStart) const noexcept
    {
        return Range(newStart, jmax(newStart, end));
    }

    /** Returns a range with the same length as this one, but moved to have the given start
     * position. */
    GLUE_NODISCARD constexpr Range movedToStartAt(const ValueType newStart) const noexcept
    {
    ...
    */
    ...
    /** Returns a range with the same start position as this one, but a different end.
        ...
        will be pushed back to equal the new end point.
    */
    GLUE_NODISCARD constexpr Range withEnd(const ValueType newEnd) const noexcept
    {
     ...
     */
    GLUE_NODISCARD constexpr Range movedToEndAt(const ValueType newEnd) const noexcept
    {
    ...
    */
    ...
    /** Returns a range with the same start as this one, but a different length.
        Lengths less than zero are treated as zero.
    */
    GLUE_NODISCARD constexpr Range withLength(const ValueType newLength) const noexcept
    {
    ...
    */
    GLUE_NODISCARD constexpr Range expanded(ValueType amount) const noexcept
    {
    ...
    */
    ...
    GLUE_NODISCARD constexpr Range getIntersectionWith(Range other) const noexcept
    {
        return Range(jmax(start, other.start), jmin(end, other.end));
    }

    /** Returns the smallest range that contains both this one and the other one. */
    GLUE_NODISCARD constexpr Range getUnionWith(Range other) const noexcept
    {
    ...
    GLUE_NODISCARD constexpr Range getUnionWith(const ValueType valueToInclude) const noexcept
    {
    ...
    */
    Range constrainRange(Range rangeToConstrain) const noexcept
    {
        ...
        return getLength() <= otherLen ? *this
                                       : rangeToConstrain.movedToStartAt(jlimit(
                                             start, end - otherLen, rangeToConstrain.getStart()));
    }
    ...
    static Range findMinAndMax(const ValueType* values, Integral numValues) noexcept
    {
        ...
        while (--numValues >
               0) // (> 0 rather than >= 0 because we've already taken the first sample)
        {
