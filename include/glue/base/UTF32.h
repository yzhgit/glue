...
{
public:
    using CharType = glue_wchar;

    inline explicit UTF32(const CharType* rawPointer) noexcept
        : data(const_cast<CharType*>(rawPointer))
    {}

    inline UTF32(const UTF32& other) = default;

    inline UTF32 operator=(UTF32 other) noexcept
    {
        data = other.data;
        return *this;
    }

    inline UTF32 operator=(const CharType* text) noexcept
    {
    ...
    inline bool operator==(UTF32 other) const noexcept { return data == other.data; }
    inline bool operator!=(UTF32 other) const noexcept { return data != other.data; }
    inline bool operator<=(UTF32 other) const noexcept { return data <= other.data; }
    inline bool operator<(UTF32 other) const noexcept { return data < other.data; }
    inline bool operator>=(UTF32 other) const noexcept { return data >= other.data; }
    ...
    inline glue_wchar operator*() const noexcept { return *data; }

    /** Moves this pointer along to the next character in the string. */
    inline UTF32 operator++() noexcept
    {
        ++data;
        return *this;
    }
    ...
    inline glue_wchar getAndAdvance() noexcept { return *data++; }

    /** Moves this pointer along to the next character in the string. */
    UTF32 operator++(int) noexcept
    {
        auto temp(*this);
        ++data;
        return temp;
    ...
    inline void operator+=(int numToSkip) noexcept { data += numToSkip; }

    inline void operator-=(int numToSkip) noexcept { data -= numToSkip; }

    /** Returns the character at a given character index from the start of the string. */
    inline glue_wchar& operator[](int characterIndex) const noexcept
    {
     ...
     * characters. */
    UTF32 operator+(int numToSkip) const noexcept { return UTF32(data + numToSkip); }
     ...
     * position. */
    inline void write(glue_wchar charToWrite) noexcept { *data++ = charToWrite; }

    inline void replaceChar(glue_wchar newChar) noexcept { *data = newChar; }
    ...
    {
#if GLUE_NATIVE_WCHAR_IS_UTF32 && !GLUE_ANDROID
        return wcslen(data);
#else
        size_t n = 0;
        while (data[n] != 0) ++n;
     ...
     * is lower. */
    size_t lengthUpTo(UTF32 end) const noexcept
    {
    ...
    */
    size_t sizeInBytes() const noexcept { return sizeof(CharType) * (length() + 1); }

    /** Returns the number of bytes that would be needed to represent the given
        unicode character in this encoding format.
    */
    static size_t getBytesRequiredFor(glue_wchar) noexcept { return sizeof(CharType); }
    ...
    */
    ...
    UTF32 findTerminatingNull() const noexcept { return UTF32(data + length()); }
    ...
    /** Copies a source string to this pointer, advancing this pointer as it goes. */
    void writeAll(UTF32 src) noexcept
    {
        auto* s = src.data;
    ...
    */
...
#if GLUE_NATIVE_WCHAR_IS_UTF32 && !GLUE_ANDROID
    /** Compares this string with another one. */
    int compare(UTF32 other) const noexcept { return wcscmp(data, other.data); }
#endif
    ...
    glue_wchar toUpperCase() const noexcept { return CharacterFunctions::toUpperCase(*data); }
    /** Returns a lower-case version of the first character of this string. */
    glue_wchar toLowerCase() const noexcept { return CharacterFunctions::toLowerCase(*data); }

    /** Parses this string as a 32-bit integer. */
    int getIntValue32() const noexcept
    {
        return CharacterFunctions::getIntValue<int, UTF32>(*this);
    }
    /** Parses this string as a 64-bit integer. */
    int64 getIntValue64() const noexcept
    {
        return CharacterFunctions::getIntValue<int64, UTF32>(*this);
    }
    ...
    UTF32 findEndOfWhitespace() const noexcept
    {
    ...
    static bool isValidString(const CharType* dataToTest, int maxBytesToRead)
    {
        ...
        while (--maxBytesToRead >= 0 && *dataToTest != 0)
            if (!canRepresent(*dataToTest++)) return false;

        return true;
    }

    /** Atomically swaps this pointer for a new value, returning the previous value. */
    UTF32 atomicSwap(UTF32 newValue)
    {
        return UTF32(reinterpret_cast<Atomic<CharType*>&>(data).exchange(newValue.data));
    }
