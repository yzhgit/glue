...
struct ContainerDeletePolicy {
    static void destroy(ObjectType* object) {
        // If the line below triggers a compiler error, it means that you are
        // using an incomplete type for ObjectType (for example, a type that is
        // declared but not defined). This is a problem because then the
        // following delete is undefined behaviour. The purpose of the sizeof is
        // to capture this situation. If this was caused by a OwnedArray of a
        // forward-declared type, move the implementation of all methods trying
        // to use the OwnedArray (e.g. the destructor of the class owning it)
        // into cpp files where they can see to the definition of ObjectType.
...
/**
    ...
    For a faster but non-thread-safe version, use
   SingleThreadedReferenceCountedObject instead.

    @see RefCountedObjectPtr, ReferenceCountedArray,
...
class GLUE_API RefCountedObject {
    ...
    */
    ...
    /** Decreases the object's reference count.
        If the count gets to zero, the object will not be deleted, but this
       method will return true, allowing the caller to take care of deletion.
    */
   ...
   protected:
    ...
    /** Copying from another object does not affect this one's reference-count.
     */
    RefCountedObject& operator=(const RefCountedObject&) noexcept { return *this; }
    /** Copying from another object does not affect this one's reference-count.
     */
    RefCountedObject& operator=(RefCountedObject&&) noexcept { return *this; }
...
/**
   ...
   to - the easiest way to make a class reference-countable is to simply make it
   inherit from RefCountedObject or SingleThreadedReferenceCountedObject, but if
   you need to, you can roll your own reference-countable class by implementing
   a set of methods called incReferenceCount(), decReferenceCount(), and
   decReferenceCountWithoutDeleting(). See RefCountedObject for examples of how
...
class RefCountedObjectPtr {
    ...
    */
    ~RefCountedObjectPtr() { decIfNotNull(referencedObject); }
    ...
    */
    ...
    void reset() noexcept {
        auto oldObject = referencedObject;  // need to null the pointer before
                                            // deleting the object
    ...
    */
    ...
    /** Returns the object that this pointer references.
        The pointer returned may be null, of course.
        Note that this methods allows the compiler to be very lenient with what
       it allows you to do with the pointer, it's safer to disable this by
       setting GLUE_STRICT_REFCOUNTEDPOINTER=1, which increased type safety and
       can prevent some common slip-ups.
    */
