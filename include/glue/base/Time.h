...
public:
    ...
    */
    Time(int year, int month, int day, int hours, int minutes, int seconds = 0,
         int milliseconds = 0, bool useLocalTime = true) noexcept;
    ...
    */
    static Time GLUE_CALLTYPE getCurrentTime() noexcept;
    ...
    */
    ...
    /** Returns a string version of this date and time, using this machine's local timezone.
        ...
        @param includeSeconds   if the time is being included, this provides an option not to
       include the seconds in it
        @param use24HourClock   if the time is being included, sets whether to use am/pm or 24
                                hour notation.
        @see formatted
    */
    String toString(bool includeDate, bool includeTime, bool includeSeconds = true,
    ...
    /** Converts this date/time to a string with a user-defined format.
       ...
       decimal number [00,53]. All days in a new year preceding the first Monday are considered to
       be in week 0.
        - %x  is replaced by the locale's appropriate date representation.
        - %X  is replaced by the locale's appropriate time representation.
        - %y  is replaced by the year without century as a decimal number [00,99].
        - %Y  is replaced by the year with century as a decimal number.
        - %Z  is replaced by the timezone name or abbreviation, or by no bytes if no timezone
       information exists.
        - %%  is replaced by %.
...
GLUE_API Time operator+(Time time, RelativeTime delta) noexcept;
/** Adds a RelativeTime to a Time. */
GLUE_API Time operator+(RelativeTime delta, Time time) noexcept;

/** Subtracts a RelativeTime from a Time. */
GLUE_API Time operator-(Time time, RelativeTime delta) noexcept;
/** Returns the relative time difference between two times. */
GLUE_API const RelativeTime operator-(Time time1, Time time2) noexcept;

/** Compares two Time objects. */
GLUE_API bool operator==(Time time1, Time time2) noexcept;
/** Compares two Time objects. */
GLUE_API bool operator!=(Time time1, Time time2) noexcept;
/** Compares two Time objects. */
GLUE_API bool operator<(Time time1, Time time2) noexcept;
