...
{
public:
#if GLUE_NATIVE_WCHAR_IS_UTF16
    using CharType = wchar_t;
#else
    using CharType = int16;
#endif

    inline explicit UTF16(const CharType* rawPointer) noexcept
        : data(const_cast<CharType*>(rawPointer))
    {}

    inline UTF16(const UTF16& other) = default;

    inline UTF16 operator=(UTF16 other) noexcept
    {
        data = other.data;
        return *this;
    }

    inline UTF16 operator=(const CharType* text) noexcept
    {
    ...
    inline bool operator==(UTF16 other) const noexcept { return data == other.data; }
    inline bool operator!=(UTF16 other) const noexcept { return data != other.data; }
    inline bool operator<=(UTF16 other) const noexcept { return data <= other.data; }
    inline bool operator<(UTF16 other) const noexcept { return data < other.data; }
    inline bool operator>=(UTF16 other) const noexcept { return data >= other.data; }
    ...
    UTF16 operator++() noexcept
    {
        auto n = (uint32) (uint16) *data++;

        if (n >= 0xd800 && n <= 0xdfff && ((uint32) (uint16) *data) >= 0xdc00) ++data;
     ...
     * characters. */
    UTF16 operator+(int numToSkip) const noexcept
    {
        auto p(*this);
        p += numToSkip;
        return p;
     ...
     * position. */
    void write(glue_wchar charToWrite) noexcept
    {
     ...
     * is lower. */
    size_t lengthUpTo(UTF16 end) const noexcept
    {
    ...
    */
    size_t sizeInBytes() const noexcept { return sizeof(CharType) * (findNullIndex(data) + 1); }

    /** Returns the number of bytes that would be needed to represent the given
        unicode character in this encoding format.
    */
    static size_t getBytesRequiredFor(glue_wchar charToWrite) noexcept
    {
    ...
    */
    ...
    static size_t getBytesRequiredFor(CharPointer text) noexcept
    {
        ...
        while ((n = text.getAndAdvance()) != 0) count += getBytesRequiredFor(n);

        return count;
    }

    /** Returns a pointer to the null character that terminates this string. */
    UTF16 findTerminatingNull() const noexcept
    {
        auto* t = data;
    ...
    /** Copies a source string to this pointer, advancing this pointer as it goes. */
    void writeAll(UTF16 src) noexcept
    {
        auto* s = src.data;
    ...
    */
    ...
    int compareIgnoreCase(UTF16 other) const noexcept { return _wcsicmp(data, other.data); }

    int compareIgnoreCaseUpTo(UTF16 other, int maxChars) const noexcept
    {
        return _wcsnicmp(data, other.data, (size_t) maxChars);
    }

    int indexOf(UTF16 stringToFind) const noexcept
    {
    ...
    int indexOf(glue_wchar charToFind) const noexcept
    {
        return CharacterFunctions::indexOfChar(*this, charToFind);
    }

    /** Returns the character index of a unicode character, or -1 if it isn't found. */
    int indexOf(glue_wchar charToFind, bool ignoreCase) const noexcept
    {
    ...
    int getIntValue32() const noexcept
    {
        ...
        return _wtoi(data);
#else
        return CharacterFunctions::getIntValue<int, UTF16>(*this);
#endif
    }
    ...
    int64 getIntValue64() const noexcept
    {
        ...
        return _wtoi64(data);
#else
        return CharacterFunctions::getIntValue<int64, UTF16>(*this);
#endif
    }
    ...
    UTF16 findEndOfWhitespace() const noexcept
    {
    ...
    static bool isValidString(const CharType* dataToTest, int maxBytesToRead)
    {
        ...
        while (--maxBytesToRead >= 0 && *dataToTest != 0)
        {
            ...
            if (n >= 0xd800)
            {
                ...
                if (n <= 0xdfff)
                {
                    if (n > 0xdc00) return false;

                    auto nextChar = (uint32) (uint16) *dataToTest++;

                    if (nextChar < 0xdc00 || nextChar > 0xdfff) return false;
                }
            }
        }
    ...
    UTF16 atomicSwap(UTF16 newValue)
    {
        return UTF16(reinterpret_cast<Atomic<CharType*>&>(data).exchange(newValue.data));
    }
    ...
    /** Returns true if the first pair of bytes in this pointer are the UTF16 byte-order mark (big
       endian). The pointer must not be null, and must contain at least two valid bytes.
    */
    ...
    /** Returns true if the first pair of bytes in this pointer are the UTF16 byte-order mark
       (little endian). The pointer must not be null, and must contain at least two valid bytes.
    */
