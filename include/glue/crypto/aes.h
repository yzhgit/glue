...
GLUE_API int aes_decrypt_cbc(const uint8_t in[],
                             size_t in_len,
                             uint8_t out[],
                             const uint32_t key[],
                             int keysize,
...
GLUE_API int aes_encrypt_ccm(
    ...
    uint8_t ciphertext[],                // OUT - Ciphertext, a concatination of the plaintext
                                         // and the MAC.
    uint32_t* ciphertext_len,            // OUT - The length of the ciphertext, always
                                         // plaintext_len + mac_len.
    uint32_t mac_len,                    // IN  - The desired length of the MAC, must be 4, 6, 8,
                                         // 10, 12, 14, or 16.
...
GLUE_API int aes_decrypt_ccm(
    const uint8_t ciphertext[],  // IN  - Ciphertext, the concatination of
    ...
    const uint8_t nonce[],       // IN  - The Nonce to use for decryption, same one
                                 // as for encryption.
    unsigned short nonce_len,    // IN  - Nonce length in bytes.
