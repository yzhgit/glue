...
#if defined(GL_ARCH_BIG_ENDIAN)
    #define GL_ENDIAN_N2L(x) reverse_bytes(x)
    #define GL_ENDIAN_L2N(x) reverse_bytes(x)
    #define GL_ENDIAN_N2B(x) (x)
    #define GL_ENDIAN_B2N(x) (x)

#elif defined(GL_ARCH_LITTLE_ENDIAN)
    #define GL_ENDIAN_N2L(x) (x)
    #define GL_ENDIAN_L2N(x) (x)
    #define GL_ENDIAN_N2B(x) reverse_bytes(x)
    #define GL_ENDIAN_B2N(x) reverse_bytes(x)
 ...
 * @return i0 || i1 || i2 || i3
 */
inline constexpr uint32_t make_uint32(uint8_t i0, uint8_t i1, uint8_t i2,
                                      uint8_t i3) {
 ...
 */
inline constexpr uint64_t make_uint64(uint8_t i0, uint8_t i1, uint8_t i2,
                                      uint8_t i3, uint8_t i4, uint8_t i5,
                                      uint8_t i6, uint8_t i7) {
    return (
        (static_cast<uint64_t>(i0) << 56) | (static_cast<uint64_t>(i1) << 48) |
...
#if defined(GL_ENDIAN_N2B)
    uint16_t x;
    typecast_copy(x, in);
    return GL_ENDIAN_N2B(x);
#else
...
#if defined(GL_ENDIAN_N2L)
    uint16_t x;
    typecast_copy(x, in);
    return GL_ENDIAN_N2L(x);
#else
 ...
 */
template <typename T>
inline void load_le(const uint8_t in[], T &x0, T &x1, T &x2, T &x3, T &x4,
                    T &x5, T &x6, T &x7) {
    x0 = load_le<T>(in, 0);
    x1 = load_le<T>(in, 1);
    x2 = load_le<T>(in, 2);
...
inline void load_le(T out[], const uint8_t in[], size_t count) {
    if (count > 0) {
#if defined(GL_ARCH_LITTLE_ENDIAN)
        typecast_copy(out, in, count);

#elif defined(GL_ARCH_BIG_ENDIAN)
        typecast_copy(out, in, count);
 ...
 */
template <typename T>
inline void load_be(const uint8_t in[], T &x0, T &x1, T &x2, T &x3, T &x4,
                    T &x5, T &x6, T &x7) {
    x0 = load_be<T>(in, 0);
    x1 = load_be<T>(in, 1);
    x2 = load_be<T>(in, 2);
...
#if defined(GL_ENDIAN_B2N)
    uint32_t o = GL_ENDIAN_B2N(in);
    typecast_copy(out, o);
#else
    out[0] = get_byte(0, in);
    out[1] = get_byte(1, in);
...
#if defined(GL_ENDIAN_L2N)
    uint32_t o = GL_ENDIAN_L2N(in);
    typecast_copy(out, o);
#else
    out[0] = get_byte(3, in);
    out[1] = get_byte(2, in);
 ...
 */
template <typename T>
inline void store_le(uint8_t out[], T x0, T x1, T x2, T x3, T x4, T x5, T x6,
                     T x7) {
    store_le(x0, out + (0 * sizeof(T)));
    store_le(x1, out + (1 * sizeof(T)));
    store_le(x2, out + (2 * sizeof(T)));
 ...
 */
template <typename T>
inline void store_be(uint8_t out[], T x0, T x1, T x2, T x3, T x4, T x5, T x6,
                     T x7) {
    store_be(x0, out + (0 * sizeof(T)));
    store_be(x1, out + (1 * sizeof(T)));
    store_be(x2, out + (2 * sizeof(T)));
...
void copy_out_vec_be(uint8_t out[], size_t out_bytes,
...
template <typename T, typename Alloc>
void copy_out_vec_le(uint8_t out[], size_t out_bytes,
                     const std::vector<T, Alloc> &in) {
